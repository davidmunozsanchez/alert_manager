...
def test_dag_cycles(dagbag):
    ...
    for dag_id, dag in dagbag.dags.items():
        ...
        assert dag_id not in dagbag.import_errors, \
            f"DAG {dag_id} has a cycle or import error"
...
def test_dag_default_args(dagbag):
    ...
    for dag_id, dag in dagbag.dags.items():
        if dag.default_args is None:
            ...
            assert dag.owner is not None and dag.owner != '', \
                f"DAG {dag_id} has no default_args and no owner"
    ...
    for dag_id, dag in dagbag.dags.items():
        for task in dag.tasks:
            ...
            retries = task.retries if task.retries is not None else \
                      dag.default_args.get('retries') if dag.default_args else None
            
            if retries is None:
                warnings.warn(UserWarning(f"Task {task.task_id} in DAG {dag_id} has no retry configuration"))
            else:
                assert retries >= 0, \
                    f"Task {task.task_id} in DAG {dag_id} has negative retries"
...
def test_dag_tasks_have_operators(dagbag):
    ...
    valid_operators = [
        ...
        'BranchPythonOperator', 'ShortCircuitOperator', 'EmailOperator',
        'PostgresOperator', 'SqliteOperator', 'HttpOperator', 'PythonSensor',
        'FileSensor', 'SqlSensor', 'TimeDeltaSensor'
    ]
    ...
    for dag_id, dag in dagbag.dags.items():
        ...
        if dag.schedule_interval is not None:
            assert dag.schedule_interval != '', \
                f"DAG {dag_id} has empty schedule_interval"
